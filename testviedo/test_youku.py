...
def getVideoInfo(url):
    ...
    videoTitle = ruleTitle.findall(urllib.urlopen(url).read().decode('utf8'))
    videoId = ruleId.findall(url)
    print videoTitle[0], videoId[0]
    return videoTitle[0], videoId[0]


def getTrueLink(videoid):
    data = urllib.urlopen('http://v.youku.com/player/getPlayList/VideoIDS/' + videoid)
    print data.read()
...
def down2file(urls, filename):
    ...
    for url in urls:
        print url
        count += 1
        print('downloading file %d/%requestd' % (count, fileNum))
